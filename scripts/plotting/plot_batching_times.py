...
def plot_batching_update_subplot(df, model, compute_type, mean_or_median):
    ...
    plt.tight_layout()
    plt.savefig(
        f'batching_paper/figs/profiling_100k_{model}_{compute_type}_aflow_left_qm9_right_{mean_or_median}.png',
        dpi=600)
    plt.show()
...
def plot_recompilation_bar_plot(df):
    ...
    plt.tight_layout()
    plt.savefig(
        'batching_paper/figs/recompilation_count_data_{dataset}.png',
        dpi=600)
    plt.show()
